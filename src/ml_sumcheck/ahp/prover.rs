...
impl<F: Field> AHPForMLSumcheck<F> {
    ...
    pub fn prove_round(mut prover_state: ProverState<F>, v_msg: &Option<VerifierMsg<F>>)
        ...
        if prover_state.round > prover_state.nv {
            return Err(crate::Error::InvalidOperationError(Some("Prover is not active".into())));
        }
        ...
        Ok(
            (ProverMsg{evaluations: products_sum}, prover_state)
        )
    }
}