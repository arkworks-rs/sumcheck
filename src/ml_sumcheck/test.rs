...
fn random_product<F: Field, R: RngCore>(nv: usize, num_multiplicands: usize, rng: &mut R)
    ...
    return (
        multiplicands.into_iter()
            .map(|x|MLExtensionArray::from_vec(x).unwrap())
            .collect(), sum)
}

fn random_combination<F: Field, R: RngCore>(nv: usize,
...
fn test_polynomial(nv: usize, num_multiplicands_range: (usize, usize), num_products: usize) {
    ...
    type F = TestField;
    let (comb, asserted_sum) = random_combination::<F, _>(nv,
                                    num_multiplicands_range,
                                    num_products,
                                    &mut rng);

    let (index_pk, index_vk) = MLSumcheck::index(&comb)
    ...
    assert!(comb.eval_at(&subclaim.point).expect("fail to evaluate") == subclaim.expected_evaluation, "wrong subclaim");
}
...
fn test_ahp(nv: usize, num_multiplicands_range: (usize, usize), num_products: usize) {
    ...
    for _ in 0..index.num_variables {
        let result = AHPForMLSumcheck::prove_round(prover_state, &verifier_msg).expect("fail to prove");
        prover_state = result.1;
        let (verifier_msg2, verifier_state2) = AHPForMLSumcheck::verify_round(&result.0,
                                                                  verifier_state,
                                                                  &mut rng)
